...
export const doctorDetailStyles = {
  // Layout styles
  pageContainer: "min-h- font-serif bg-linear-to-br from-med-lightest via-white to-med-lightest p-4 sm:p-6",
  ...
  headerIconContainer: "p-3 bg-med-vibrant rounded-full shadow-lg",
  headerTitle: "text-3xl sm:text-4xl md:text-5xl font-bold bg-linear-to-r from-med-vibrant to-med-vibrant bg-clip-text text-transparent mt-2 sm:mt-0",
  
  // Form styles
  formContainer: "bg-white/80 backdrop-blur-sm border border-med-light shadow-2xl p-6 sm:p-8 rounded-3xl mb-16",
  formGrid: "grid grid-cols-1 md:grid-cols-2 gap-6",
  
  // Input styles
  inputBase: "p-3 rounded-full border-2 border-med-light bg-white placeholder:text-gray-400 shadow-sm w-full focus:outline-none focus:border-med-vibrant focus:ring-2 focus:ring-med-light transition-all",
  textareaBase: "p-3 rounded-xl border-2 border-med-light bg-white placeholder:text-gray-400 shadow-sm w-full focus:outline-none focus:border-med-vibrant focus:ring-2 focus:ring-med-light transition-all",
  inputWithIcon: "pr-12",
  
  // File upload styles
  fileInput: "w-40 md:w-[180px] border-2 border-med-light rounded-full p-2 text-sm bg-white focus:border-med-vibrant transition",
  imagePreview: "h-12 w-12 md:h-16 md:w-16 rounded-full shadow-md border-2 border-med-soft object-cover",
  ...
  scheduleContainer: "p-6 bg-med-lightest rounded-2xl border border-med-light",
  scheduleHeader: "flex items-center gap-3 mb-4",
  scheduleTitle: "text-lg font-semibold text-med-dark",
  scheduleInputsContainer: "flex flex-wrap items-center gap-3",
  scheduleDateInput: "p-3 rounded-full border-2 border-med-light w-full sm:w-auto",
  scheduleTimeSelect: "p-3 rounded-full border-2 border-med-light w-full sm:w-auto",
  addSlotButton: "px-5 py-3 bg-med-vibrant text-white rounded-full flex items-center gap-2 w-full sm:w-auto justify-center",
  slotsGrid: "mt-4 space-y-2 max-w-9xl grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 lg:grid-cols-3 gap-4",
  slotItem: "flex justify-between items-center bg-med-lightest p-3 rounded-xl border border-med-light shadow-sm hover:shadow-md text-med-dark",
  ...
  submitButtonEnabled: "bg-linear-to-r from-med-vibrant to-med-vibrant text-white",
  ...
  toastSuccess: "bg-med-lightest border border-med-soft text-med-dark",
...
export const pageStyles = {
  container: "min-h-screen font-serif bg-med-lightest p-3 sm:p-4 md:p-6 lg:p-8",
  ...
  headerTitle: "text-xl sm:text-2xl font-semibold text-med-dark",
  headerSubtitle: "text-xs sm:text-sm text-med-vibrant",
  ...
  searchIcon: "text-med-vibrant",
  searchInput: "ml-3 w-full outline-none text-med-dark placeholder-teal-400 bg-transparent text-sm",
  ...
  dateFilterIcon: "text-med-vibrant",
  dateInput: "text-sm outline-none text-med-dark bg-transparent w-full",
  
  selectFilter: "text-sm px-3 py-2 cursor-pointer rounded-full bg-med-light shadow-sm outline-teal-300 w-full sm:w-auto",
  clearButton: "ml-0 sm:ml-2 px-3 cursor-pointer py-2 rounded-full bg-med-vibrant text-white text-sm shadow-sm hover:opacity-95 transition w-full sm:w-auto",
  
  loadingErrorContainer: "col-span-full text-center text-med-vibrant py-12 rounded-lg bg-white/60 border border-med-light",
  errorContainer: "col-span-full text-center text-rose-600 py-6 rounded-lg bg-white/60 border border-rose-100",
  noResultsContainer: "col-span-full text-center text-med-vibrant py-12 rounded-lg bg-white/60 border border-med-light",
  
  gridContainer: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4",
  
  card: "bg-white rounded-2xl p-3 sm:p-4 md:p-5 shadow-sm border border-med-light flex flex-col gap-3 hover:shadow-md transform hover:-translate-y-1 transition",
  cardHeader: "flex items-start lg:line-clamp-2 justify-between gap-3",
  cardTitle: "text-base sm:text-lg font-medium text-med-dark truncate",
  patientInfo: "text-xs sm:text-sm text-med-vibrant flex items-center gap-2",
  doctorInfo: "mt-1 text-xs sm:text-sm text-med-vibrant truncate",
  doctorSpeciality: "font-medium text-med-dark",
  
  feeLabel: "text-nd lg:pt-3 lg:justify-start flex items-center font-bold text-med-dark text-xs sm:text-sm",
  feeAmount: "text-lg sm:text-xl font-semibold lg:justify-start text-med-dark flex items-center justify-end gap-1",
  
  slotContainer: "inline-flex items-center gap-2 text-sm text-med-vibrant bg-med-lightest px-3 py-1 rounded-full",
  slotIcon: "text-med-vibrant",
  ...
  showMoreButton: "px-4 py-2 cursor-pointer rounded-full bg-white border border-med-soft shadow-sm hover:shadow-md transition",
};

export const statusClasses = (status) => {
  const s = (status || "").toLowerCase();
  if (s === "confirmed")
    return "bg-med-lightest text-med-dark border border-med-light";
  if (s === "completed")
    return "bg-med-lightest text-med-dark border border-med-light";
  if (s === "rescheduled")
...
export const addServiceStyles = {
  // Container styles
  container: {
    main: "min-h-screen font-serif bg-linear-to-br from-med-lightest via-med-light to-med-lightest relative flex items-center justify-center p-4 sm:p-6 overflow-x-hidden",
    form: "w-full max-w-5xl bg-white/90 backdrop-blur-sm rounded-3xl p-6 sm:p-8 shadow-2xl border border-med-light/50 box-border"
  },

  // Header styles
  header: {
    title: "text-2xl sm:text-3xl font-extrabold text-transparent bg-linear-to-r from-med-dark to-med-vibrant bg-clip-text",
    subtitle: "text-sm text-gray-500 mt-1"
  },

  // Button styles
  buttons: {
    reset: "w-full sm:w-auto px-4 py-2 cursor-pointer rounded-full bg-white border border-med-light hover:shadow transition-shadow duration-200",
    submit: "inline-flex justify-center items-center gap-2 w-full sm:w-auto px-5 py-2 rounded-full bg-linear-to-r from-med-vibrant to-med-vibrant text-white font-medium hover:shadow-lg transition-all duration-200 disabled:opacity-60",
    addInstruction: "inline-flex cursor-pointer items-center gap-2 px-3 py-1.5 rounded-full bg-med-lightest border border-med-soft hover:bg-med-light transition-colors",
    uploadImage: "flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full bg-white border border-med-soft hover:shadow transition-shadow",
    removeImage: "px-3 py-2 rounded-full bg-white border border-red-100 hover:shadow transition-shadow",
    addSlot: "w-full inline-flex items-center justify-center gap-2 px-4 py-3 rounded-full bg-linear-to-r from-med-vibrant to-med-vibrant text-white font-medium hover:shadow-lg transition-all duration-200",
    ...
    container: (hasError) => 
      `w-full rounded-2xl p-4 ${
        hasError 
          ...
          : "bg-linear-to-b from-med-lightest to-med-lightest border border-med-light"
      } shadow-inner flex flex-col items-center gap-4`,
    preview: "w-full h-56 rounded-xl overflow-hidden bg-white flex items-center justify-center border border-med-light",
    placeholder: "flex flex-col items-center text-med-vibrant"
  },
  ...
  formFields: {
    input: (hasError) => 
      `mt-2 w-full px-4 py-3 rounded-full focus:outline-none focus:ring-2 shadow-md transition-all ${
        hasError 
          ...
          : "border border-med-light focus:ring-med-soft"
      }`,
    textarea: (hasError) => 
    ...
    select: "mt-2 w-full px-4 py-3 rounded-full border border-med-light text-gray-700 bg-white appearance-none focus:ring-2 focus:ring-med-soft focus:outline-none",
    smallSelect: "mt-1 w-full px-3 py-2 rounded-full border border-med-light text-gray-700 bg-white appearance-none focus:ring-2 focus:ring-med-soft focus:outline-none",
    timeSelect: "mt-1 w-full px-2 py-2 rounded-full border border-med-light text-gray-700 bg-white appearance-none focus:ring-2 focus:ring-med-soft focus:outline-none",
    ampmSelect: "mt-1 w-full px-1 py-2 rounded-full border border-med-light text-gray-700 bg-white appearance-none focus:ring-2 focus:ring-med-soft focus:outline-none"
  },
  ...
  instructions: {
    ...
    item: "flex items-start gap-3 my-2 bg-white rounded-full p-3 border border-med-lightest shadow-sm hover:shadow transition-shadow min-w-0",
    input: "flex-1 min-w-0 px-3 py-2 rounded-full border border-med-light focus:outline-none focus:ring-2 focus:ring-med-soft",
  ...
  slots: {
    container: (hasError) => 
      `bg-linear-to-br from-white to-med-lightest rounded-2xl p-4 ${
        hasError 
          ? "border-2 border-red-200" 
          : "border border-med-lightest"
      } shadow-sm`,
    slotItem: "flex items-center gap-2 bg-linear-to-r from-med-lightest to-med-lightest border border-med-light px-2 py-2 my-1 rounded-full shadow hover:shadow-md transition-shadow min-w-0",
  ...
  toast: {
    ...
    toastInfo: "bg-linear-to-r from-med-lightest to-med-lightest border-med-light",
    toastSuccess: "bg-linear-to-r from-med-lightest to-med-lightest border-med-light",
    iconContainer: (type) => {
      const styles = {
        error: "bg-linear-to-r from-red-100 to-orange-100 text-red-600",
        info: "bg-linear-to-r from-med-light to-med-light text-med-vibrant",
        success: "bg-linear-to-r from-med-light to-med-light text-med-vibrant"
      };
  ...
  // Label styles
  labels: {
    standard: "text-sm font-medium text-med-dark",
  ...
  icon: {
    number: "font-semibold text-med-vibrant",
    clock: "w-4 h-4 xl:w-6 xl:h-6 text-med-vibrant",
...
export const doctorListStyles = {
  container: "min-h-screen font-serif bg-med-lightest p-4 sm:p-6 md:p-8",
  ...
  headerIconSvg: "text-med-vibrant",
  headerTitle: "text-base sm:text-lg font-semibold text-med-dark",
  headerSubtitle: "text-sm sm:text-md text-med-vibrant",
  ...
  searchIcon: "text-med-vibrant",
  searchInput: "ml-3 w-full outline-none text-med-dark placeholder-teal-400 bg-transparent",
  clearButton: "px-3 py-2 cursor-pointer rounded-full bg-med-vibrant text-white shadow hover:opacity-95 transition w-full sm:w-auto",
  ...
  filterButton: (isActive, color) => {
    ...
    if (color === 'emerald') {
      return isActive
        ? `${base} bg-med-vibrant text-white border-med-vibrant`
        : `${base} bg-white text-med-dark border-med-soft`;
    } else if (color === 'red') {
      return isActive
  ...
  loadingContainer: "text-center text-med-vibrant py-8",
  noResultsContainer: "text-center text-med-vibrant py-8",
  
  gridContainer: "max-w-6xl grid xl:grid-cols-2 lg:grid-cols-2 lg:gap-3 xl:gap-4 mx-auto space-y-4",
  
  article: "bg-linear-to-r from-med-light/50 to-white rounded-2xl shadow-md border border-med-light overflow-hidden transition-all duration-300",
  articleContent: "flex flex-col sm:flex-row items-start sm:items-center gap-4 p-3 sm:p-4 md:p-5",
  
  doctorImage: "w-16 h-16 sm:w-20 sm:h-20 rounded-xl object-cover border border-med-soft shadow-sm mx-auto sm:mx-0",
  ...
  doctorName: "text-base sm:text-lg md:text-xl text-med-dark font-medium truncate",
  ...
  availabilityBadge: (isAvailable) => 
    `ml-0 sm:ml-2 mt-2 sm:mt-0 inline-flex items-center gap-2 text-xs font-medium px-2 py-0.5 rounded-full ${
      isAvailable
        ? "bg-med-lightest text-med-dark"
        : "bg-red-50 text-red-600"
    }`,
  availabilityDot: (isAvailable) => 
    `w-2 h-2 rounded-full ${isAvailable ? "bg-med-vibrant" : "bg-red-600"}`,
  
  doctorDetails: "text-sm text-med-vibrant truncate mt-2 sm:mt-1",
  
  ratingContainer: "flex items-center gap-3 mt-3 sm:mt-0 sm:ml-4",
  rating: "text-sm text-med-dark flex items-center gap-1",
  toggleButton: (isOpen) => 
  ...
  statsLabel: "text-xs text-med-vibrant",
  statsValue: "text-sm text-med-dark font-medium flex items-center gap-2",
  ...
  feesLabel: "text-md font-bold text-med-dark",
  feesValue: "text-sm text-med-dark font-medium flex items-center gap-1",
  
  expandableContent: "px-4 md:px-5 bg-white overflow-auto sm:overflow-visible",
  
  aboutSection: "col-span-2",
  aboutHeading: "text-md font-bold text-med-dark mb-1",
  aboutText: "text-sm text-med-vibrant break-words whitespace-normal",
  
  qualificationsHeading: "text-md text-med-dark font-bold",
  qualificationsText: "text-sm text-med-vibrant break-words whitespace-normal",
  
  scheduleHeading: "text-md text-med-dark font-bold",
  scheduleDate: "text-xs text-med-vibrant",
  scheduleSlot: "text-xs px-3 py-1 rounded-full border border-med-light shadow-sm break-words",
  
  statsSidebar: "col-span-1 flex flex-col sm:flex-row md:flex-col xl:flex-col lg:flex-col gap-3 items-start md:items-end",
  statsItemHeading: "text-md text-med-dark font-bold",
  statsItemValue: "text-sm text-med-dark",
  locationValue: "text-sm sm:whitespace-nowrap whitespace-normal text-med-dark",
  
  showMoreButton: "px-5 py-2 cursor-pointer rounded-full bg-white border border-med-soft shadow-sm hover:shadow-md transition",
...
export const heroStyles = {
  // Container styles
  container: "min-h-screen font-sans bg-linear-to-b from-med-lightest to-white",
  ...
  decorativeBg: {
    ...
    blurShape: "w-full h-44 md:h-56 rounded-3xl bg-med-light/60 blur-3xl"
  },
  
  // Content container
  contentBox: "bg-white/90 backdrop-blur-sm border border-med-light rounded-3xl shadow-xl p-8 md:p-12 text-center",
  ...
  // Text styles
  heading: "text-3xl md:text-4xl font-extrabold text-med-dark mb-2",
  ...
  infoCards: {
    ...
    card: "p-4 rounded-lg bg-med-lightest border border-med-light text-left",
    cardTitle: "font-semibold text-med-dark",
...
export const serviceListStyles = {
  // Layout styles
  pageContainer: "p-4 sm:p-6 max-w-6xl font-serif mx-auto min-h-screen bg-linear-to-b from-med-lightest via-white to-white",
  
  // Header styles
  headerContainer: "flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4",
  headerTitle: "text-2xl md:text-3xl font-extrabold text-med-dark",
  headerSubtitle: "text-sm text-med-vibrant mt-1",
  
  // Filter and search styles
  filterContainer: "flex flex-col md:flex-col items-stretch md:items-center gap-3 w-full md:w-auto",
  filterButtonsContainer: "inline-flex flex-wrap items-center gap-2 rounded-full border border-med-light bg-white p-1",
  filterButton: "px-3 py-1 cursor-pointer rounded-full text-sm transition",
  filterButtonActive: "bg-med-vibrant text-white",
  filterButtonInactive: "text-med-dark bg-transparent",
  searchContainer: "relative w-full md:w-auto",
  searchIcon: "absolute inset-y-0 left-3 flex items-center pointer-events-none",
  searchIconSvg: "w-5 h-5 text-med-soft",
  searchInput: "pl-12 pr-4 py-2 rounded-full border border-med-light shadow-sm focus:outline-none focus:ring-2 focus:ring-med-soft transition w-full md:w-72 bg-white",
  
  // Grid styles
  servicesGrid: "grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2 gap-6",
  
  // Service card styles
  serviceCard: "bg-white rounded-2xl overflow-hidden transform transition hover:-translate-y-1 hover:shadow-2xl border border-med-lightest",
  serviceCardContent: "flex flex-col sm:flex-row sm:items-start gap-4 p-4 cursor-pointer",
  serviceImageContainer: "w-full sm:w-20 h-40 sm:h-20 rounded-lg overflow-hidden bg-med-lightest ring-1 ring-med-lightest flex-shrink-0",
  serviceImage: "w-full h-full object-cover",
  serviceImagePlaceholder: "w-full h-full flex items-center justify-center text-med-soft",
  serviceInfoContainer: "flex-1 min-w-0",
  serviceHeader: "flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2",
  serviceName: "text-lg font-semibold text-med-dark truncate",
  serviceDescription: "text-sm text-med-vibrant mt-1 line-clamp-2",
  servicePriceContainer: "text-left sm:text-right mt-2 sm:mt-0",
  servicePrice: "text-md font-semibold text-med-dark",
  availabilityBadge: "text-xs mt-1 inline-flex items-center gap-1 px-2 py-1 rounded-full",
  availabilityAvailable: "bg-med-lightest text-med-dark",
  availabilityUnavailable: "bg-rose-50 text-rose-700",
  slotsInfo: "mt-2 flex items-center gap-2 font-bold text-sm text-med-vibrant",
  chevronContainer: "pl-3 self-start sm:self-center",
  chevronIcon: "w-6 h-6 transition-transform",
  chevronOpen: "rotate-180 text-med-vibrant",
  chevronClosed: "text-med-soft",
  ...
  editImagePreview: "w-full sm:w-36 h-36 rounded-lg overflow-hidden bg-med-lightest ring-1 ring-med-lightest flex-shrink-0",
  editFormFields: "flex-1 space-y-2",
  inputBase: "w-full border rounded-lg px-3 py-2 outline-none transition focus:ring-2 focus:ring-med-soft focus:border-med-soft border-med-light bg-white",
  availabilitySelectContainer: "mt-1 flex items-center gap-2",
  availabilityLabel: "text-sm text-med-vibrant",
  availabilitySelect: "border rounded-full cursor-pointer px-3 py-1 outline-none focus:ring-2 focus:ring-med-soft border-med-soft",
  fileInputContainer: "mt-2",
  fileInputLabel: "text-sm block mb-1 text-med-dark",
  fileInput: "w-full border border-med-soft rounded-full px-4 py-2 text-sm bg-white cursor-pointer file:bg-med-lightest file:border-0 file:px-4 file:py-1 file:rounded-full file:text-med-dark hover:border-med-vibrant focus:outline-none focus:ring-2 focus:ring-med-soft transition",
  textarea: "min-h-[68px]",
  textareaInstructions: "min-h-[80px]",
  formLabel: "block text-md font-bold mb-1 text-med-vibrant",
  formLabelSmall: "block text-sm text-med-vibrant",
  
  // Slots management
  slotsHeader: "flex items-center justify-between",
  addSlotButton: "inline-flex cursor-pointer items-center gap-2 text-sm px-2 py-1 rounded-full border border-med-light",
  slotsContainer: "space-y-2 mt-2",
  slotRow: "flex flex-col sm:flex-row sm:items-center gap-2 w-full",
  slotDateInput: "border rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-med-soft border-med-light w-full sm:w-auto",
  slotTimeContainer: "flex gap-2 items-center w-full sm:w-auto",
  slotSelect: "border rounded-lg px-2 py-1 outline-none focus:ring-2 focus:ring-med-soft border-med-light w-20",
  ...
  saveButton: "px-3 py-2 rounded-full cursor-pointer bg-med-vibrant text-white w-full sm:w-auto",
  
  // View mode styles
  viewSection: "space-y-3",
  viewSectionTitle: "text-md font-bold text-med-dark",
  viewSectionContent: "text-md text-med-vibrant mt-1",
  instructionsList: "list-disc list-inside text-md text-med-vibrant mt-1 space-y-1",
  slotsList: "mt-2 space-y-2 text-sm text-med-vibrant",
  noSlotsMessage: "text-med-soft",
  slotItem: "flex font-bold items-center gap-3",
  slotIcon: "w-4 h-4 text-med-vibrant",
  
  // Action buttons (view mode)
  viewActions: "flex items-center gap-2 justify-end",
  editButton: "inline-flex bg-med-soft cursor-pointer items-center gap-2 px-3 py-2 rounded-full border border-med-soft",
  removeButton: "inline-flex items-center bg-red-200 cursor-pointer gap-2 px-3 py-2 rounded-full border text-rose-600",
  
  // Empty state
  emptyState: "text-center text-med-soft mt-8",
  ...
  toastSuccess: "bg-white border-med-light",
  toastError: "bg-white border-slate-100",
  toastContent: "flex items-start gap-3",
  toastIconSuccess: "text-med-vibrant",
  toastIconError: "text-slate-400",
  toastIconSvg: "w-5 h-5",
  toastMessage: "flex-1 text-sm text-med-dark",
  toastCloseButton: "text-med-soft",
  toastCloseIcon: "w-4 h-4",
  
  // Text colors
  textEmerald300: "text-med-soft",
  textEmerald400: "text-med-vibrant",
  textEmerald500: "text-med-vibrant",
  textEmerald600: "text-med-vibrant",
  textEmerald700: "text-med-dark",
  textRose600: "text-rose-600",
...
export const serviceAppointmentsStyles = {
  ...
  searchIcon: "w-4 h-4 text-med-vibrant",
  searchInput: "pl-10 pr-10 w-full rounded-full border border-med-soft bg-white/90 py-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-med-soft transition-all",
  clearSearchButton: "absolute right-3 rounded-full p-1 hover:bg-gray-100",
  clearSearchIcon: "w-4 h-4 text-gray-500",
  
  statusFilterSelect: "text-sm px-3 py-2 cursor-pointer rounded-full border border-med-vibrant",
  searchInfo: "mt-2 text-xs text-gray-500 flex items-center justify-between",
  refreshButton: "text-xs text-med-vibrant hover:underline",
  
  loadingContainer: "col-span-full rounded-2xl p-8 bg-white/90 border border-med-lightest shadow-sm flex items-center justify-center gap-3",
  errorContainer: "col-span-full rounded-2xl p-4 bg-rose-50 border border-rose-100 text-rose-700",
  
  noResultsContainer: "col-span-full rounded-2xl p-8 bg-white/90 border border-med-lightest shadow-sm flex items-center justify-center flex-col gap-3",
  noResultsIcon: "text-3xl text-med-soft",
  ...
  cardInner: "card-inner rounded-2xl overflow-hidden border-2 border-med-soft/60 p-5 bg-white/90 shadow-lg h-full flex flex-col justify-between",
  ...
  patientAvatar: "rounded-full w-12 h-12 flex items-center justify-center bg-med-light/70",
  patientAvatarIcon: "h-6 w-6 text-med-dark",
  patientInfo: "min-w-0",
  patientName: "text-lg md:text-sm lg:text-xs xl:text-md whitespace-nowrap font-bold leading-tight text-med-dark w-full line-clamp-2",
  ...
  legendContainer: "mt-6 p-4 rounded-lg bg-white/80 shadow-inner border border-med-light text-sm flex flex-col sm:flex-row flex-wrap items-center gap-4",
  ...
  statusBadge: (status) => {
    const map = {
      ...
      Confirmed: "bg-med-light text-med-dark",
  ...
  statusSelect: (terminal) => 
    `text-sm cursor-pointer px-3 py-1 rounded-full border focus:outline-none transition ${
      terminal
        ...
        : "bg-white text-med-dark border-med-vibrant"
    }`,
  ...
  rescheduleButton: (terminal) => 
    `text-sm px-3 py-1 rounded-full cursor-pointer border transition ${
      terminal
        ? "bg-gray-50 text-gray-400 border-gray-200 cursor-not-allowed"
        : "bg-white text-med-dark border-med-vibrant hover:shadow-sm"
    }`,
  ...
  rescheduleDateInput: "text-sm px-3 py-1 w-full sm:w-auto text-med-dark border border-med-vibrant rounded-full",
  rescheduleTimeInput: "text-sm px-3 py-1 w-full sm:w-auto text-med-dark border border-med-vibrant rounded-full",
  rescheduleActions: "flex gap-2 w-full sm:w-auto",
  rescheduleSaveButton: "flex-1 sm:flex-none px-3 py-1 bg-med-vibrant cursor-pointer text-white border-med-vibrant rounded-full text-sm",
  ...
  toast: "max-w-xs w-full rounded-lg shadow-lg px-4 py-3 border-l-4 border-med-vibrant bg-white/95 backdrop-blur-sm",
  toastContent: "flex items-start gap-3",
  toastSpinner: "h-5 w-5 animate-spin text-med-vibrant",
  toastText: "flex-1",
...
export const serviceDashboardStyles = {
  // Container styles
  container: "min-h-screen font-serif p-4 sm:p-6 bg-linear-to-b from-med-lightest via-teal-25 to-white",
  ...
  header: {
    ...
    title: "text-2xl md:text-3xl font-semibold text-med-dark",
    ...
    button: (hasServicesProp) => 
      `px-3 py-1 rounded-full text-sm ${
        hasServicesProp
          ...
          : "bg-white text-med-vibrant border border-med-soft hover:shadow-sm"
      }`
  },
  ...
  search: {
    ...
    inputContainer: "flex items-center gap-2 bg-white px-4 py-2 rounded-full shadow-sm border border-med-soft w-full sm:w-64",
  ...
  table: {
    ...
    headerMd: "hidden md:grid lg:hidden grid-cols-5 items-center gap-6 px-4 py-3 text-sm text-gray-600 bg-med-lightest",
    headerLg: "hidden lg:grid md:text-xs lg:text-xs xl:text-md grid-cols-12 items-center gap-4 px-4 py-3 text-sm text-gray-600 bg-med-lightest",
    ...
    tabletImage: "w-10 h-10 rounded-lg overflow-hidden bg-gray-200 ring-1 ring-med-light",
    tabletTextContainer: "min-w-0",
    tabletServiceName: "text-sm font-medium text-med-dark whitespace-nowrap",
    ...
    desktopImage: "w-16 h-16 rounded-xl overflow-hidden ring-1 ring-med-light bg-gray-200",
    desktopServiceName: "font-semibold md:text-xs lg:text-lg xl:text-lg text-med-dark",
    desktopCell: (span) => `col-span-${span}`,
    desktopCenterCell: (span) => `col-span-${span} text-center`,
    
    // Mobile view
    mobileView: "md:hidden flex flex-col gap-3",
    mobileImage: "w-14 h-14 rounded-lg overflow-hidden bg-gray-200 ring-1 ring-med-light",
    mobileServiceHeader: "flex items-center justify-between gap-3",
    mobileServiceName: "font-semibold text-xs text-med-dark",
  ...
  // Show more button
  showMore: {
    container: "px-6 py-4 border-t border-med-lightest flex justify-center",
    button: "px-4 py-2 rounded-full cursor-pointer bg-white border border-med-soft shadow-sm hover:bg-med-lightest transition"
  },
  
  // StatCard component styles
  statCard: {
    container: "rounded-full bg-linear-to-br from-med-light to-med-lightest shadow-sm border border-med-light p-4 gap-4 flex items-center",
    iconContainer: "w-12 h-12 rounded-full bg-med-lightest flex items-center justify-center text-med-dark",
