    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <TiltWrapper tiltMultiplier={3}>
          <div className="bg-white/90 p-6 rounded-2xl border border-slate-200/50 shadow-md h-full">
        ...
        <TiltWrapper tiltMultiplier={3}>
          <div className="bg-white/90 p-6 rounded-2xl border border-slate-200/50 shadow-md h-full">
            ...
            <div className="grid grid-cols-2 gap-4">
              <StatusBox label="Completed" count={data.statusCounts.completed} color="text-emerald-600" bg="bg-emerald-50/80" />
              <StatusBox label="Pending" count={data.statusCounts.pending} color="text-amber-600" bg="bg-amber-50/80" />
              <StatusBox label="Paid" count={data.statusCounts.paid} color="text-blue-600" bg="bg-blue-50/80" />
              <StatusBox label="Cancelled" count={data.statusCounts.cancelled} color="text-rose-600" bg="bg-rose-50/80" />
            </div>
          </div>
        </TiltWrapper>
      </div>
...
function MetricCard({ title, value, trend, trendUp, icon, colorClass }) {
  return (
    <TiltWrapper tiltMultiplier={7} className="h-full">
      <div className="bg-white/80 backdrop-blur-sm p-5 rounded-2xl border border-slate-200/50 shadow-sm flex flex-col justify-between hover:shadow-xl hover:shadow-slate-300/30 transition-all duration-300 h-full group">
        <div className="flex items-start justify-between mb-2">
          <div className={`p-2 rounded-xl ${colorClass} group-hover:scale-110 transition-transform`}>{icon}</div>
          {trend && (
