...
const CATEGORIES = ["All", "Clinical Cases", "Second Opinions", "Research & Guidelines", "Practice Management", "General Medical"];
const SUPPORT_CIRCLES = ["Surgeons Network", "Primary Care Discussion", "Specialist Consults", "Medical Students & Residents"];
...
export default function Forum() {
  ...
  const [activeCircle, setActiveCircle] = useState("Surgeons Network");
  ...
  const [newPost, setNewPost] = useState({ title: "", content: "", category: "Clinical Cases" });
  ...
  async function fetchPosts() {
    ...
    try {
      ...
      let url = "";
      if (forumTab === "circles") {
        url = `${API_BASE}/api/posts?forumType=doctor&circle=${encodeURIComponent(activeCircle)}`;
      } else {
        url = `${API_BASE}/api/posts?forumType=doctor&isQA=${forumTab === "qa"}`;
        if (category !== "All") {
  ...
  async function handleCreatePost(e) {
    ...
    try {
      const token = isSignedIn ? await getToken() : doctorToken;
      const res = await fetch(`${API_BASE}/api/posts`, {
        ...
        body: JSON.stringify({
          ...
          authorName: user?.fullName || user?.username || doctorInfo?.name || "Doctor",
          isQA: forumTab === "qa",
        ...
        <div className="bg-white/60 border border-emerald-200/60 rounded-3xl p-8 mb-8 shadow-sm backdrop-blur-md flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-slate-600 mt-2 text-sm sm:text-base leading-relaxed">
              Connect with fellow physicians, discuss clinical cases, and share medical insights securely.
            </p>
          </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-emerald-200 shadow-2xl animate-fade-in">
              ...
              <form onSubmit={handleCreatePost} className="space-y-4 font-sans">
                ...
                <div className="flex gap-3 justify-end pt-4">
                  ...
                  >
                    {submittingPost ? "Publishing..." : (forumTab === "qa" ? "Publish Case" : "Publish")}
                  </button>
                </div>
              </form>
            </div>
