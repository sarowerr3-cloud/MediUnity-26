    ...
    try {
      ...
      if (json.success) {
        ...
        if (activeTab === "chat" && document.visibilityState === "visible") {
          fetch(`${API_BASE}/api/messages/${appointmentId}/read`, {
            method: "PUT",
            headers
          }).catch(() => null);
        }
          ...
          <>
            ...
            <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-100 shrink-0 flex gap-2 items-center">
              <input
                ...
                disabled={sending}
                className="flex-grow border border-slate-200 bg-slate-50 focus:bg-white rounded-full px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800"
              />
              <button
                type="submit"
                ...
                <div className="flex gap-2">
                  <div className="flex-1 flex flex-col gap-1.5">
                    <input 
                      ...
                      className="border border-slate-200 rounded-lg px-3 py-1.5 text-xs focus:ring-1 focus:ring-emerald-500 focus:outline-none bg-slate-50 text-slate-800"
                      required
                    />
                    <input 
                      type="file" 
