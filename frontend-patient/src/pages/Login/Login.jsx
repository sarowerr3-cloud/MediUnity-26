...
export default function Login({ defaultRole }) {
  ...
  const handleMouseMove = (e) => {
    ...
    setCardStyle({
      transform: `perspective(1000px) rotateX(${-y * 5}deg) rotateY(${x * 5}deg) scale3d(1.02, 1.02, 1.02)`,
    });
  };
    ...
    <div className={`min-h-screen font-serif flex flex-col lg:flex-row relative overflow-hidden transition-all duration-700 ease-in-out ${activeClass.bgContainer}`}>
      ...
      <div className={`hidden lg:flex lg:w-5/12 p-12 flex-col justify-between relative overflow-hidden transition-all duration-700 z-10 ${
        ...
        <div className="space-y-8 my-auto relative z-10 max-w-md">
          ...
          <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-bold uppercase tracking-wider bg-white/10 backdrop-blur-md border-white/10`}>
            {isPatient ? (
              <>
                ...
                <span>{isBn ? "রোগী সেবা স্থান" : "Patient Care Space"}</span>
              </>
            ) : (
              <>
                <Stethoscope className="w-3.5 h-3.5 text-blue-300" />
                <span>{isBn ? "ক্লিনিকাল হাব" : "Clinical Operations Hub"}</span>
              </>
            )}
          </div>
          ...
          <h1 className="text-4xl font-extrabold leading-tight tracking-tight font-serif">
            {isPatient 
              ? (isBn ? "আপনার সুস্থতা যাত্রা এখানেই শুরু।" : "Your Wellness Journey Starts Here.") 
              : (isBn ? "বিশ্বমানে ক্লিনিকাল হেলথ পরিচালনা করুন।" : "Operate Clinical Health at Scale.")}
          </h1>

          {/* Paragraph */}
          <p className="text-slate-200/90 text-sm leading-relaxed font-sans">
            {isPatient 
              ? (isBn ? "পরামর্শ বুক করুন, মেডিকেল রেকর্ড দেখুন, যাচাইকৃত ক্লিনিকে সংযুক্ত হন এবং স্বাস্থ্য তথ্য পরিলাপ করুন।" : "Gain secure access to book consultations, review your personal medical records, connect with verified clinics, and monitor key vitals.") 
              : (isBn ? "ইলেকট্রনিক রেকর্ড, ডিজিটাল প্রেসক্রিপশন, ল্যাব অ্যালার্ট দেখুন এবং রোগী শিডিউলিং সহজ করুন।" : "Securely access electronic records, write digital prescriptions, review laboratory alerts, and streamline patient scheduling operations.")}
          </p>
          ...
          {isPatient ? (
            ...
            <div className="space-y-4 pt-4">
              <div className="flex items-start gap-3 bg-white/10 backdrop-blur-md border border-white/10 p-4 rounded-2xl">
                ...
                <div>
                  <h4 className="text-sm font-bold font-sans">{isBn ? "সুরক্ষিত সংক্রিপ্ত ফাইল" : "Secure Encrypted Files"}</h4>
                  <p className="text-xs text-slate-200 mt-0.5 font-sans">{isBn ? "আপনার স্বাস্থ্য তথ্য এন্ড-টু-এন্ড সুরক্ষায় সিলদ্ধ।" : "Your health data is sealed with end-to-end security protocols."}</p>
                </div>
              </div>
              ...
              <div className="bg-emerald-900/40 border border-emerald-500/20 p-4 rounded-2xl text-xs space-y-2">
                <div className="flex items-center gap-2 text-emerald-300 font-bold">
                  ...
                  <span>{isBn ? "আজের সুস্থতা টিপস" : "Wellness Tip of the Day"}</span>
                </div>
                <p className="text-slate-200 font-sans leading-relaxed">
                  {isBn ? "হাইড্রেটেড থাকুন! দিনে ৮ গ্লাস পানি পান করলে মনোযোগ বাড়ে, পেশি পুনরুদ্ধার হয় নিয়মিততা বজায় থাকে।" : "Remember to stay hydrated! Drinking 8 glasses of water boosts concentration, helps muscle recovery, and keeps vitals steady."}
                </p>
              </div>
            </div>
          ) : (
          ...
          <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/10">
            <div>
              ...
              <div className="text-[10px] text-slate-300 uppercase tracking-widest font-sans font-bold mt-1">
                {isPatient ? (isBn ? "সক্রিয় রোগী" : "Active Lives Managed") : (isBn ? "যাচাইকৃত বিশেষের সংখ্যা" : "Verified Specialties")}
              </div>
            </div>
            <div>
              <div className="text-3xl font-extrabold tracking-tight font-sans">
              ...
              <div className="text-[10px] text-slate-300 uppercase tracking-widest font-sans font-bold mt-1">
                {isPatient ? (isBn ? "গ্রাহক সন্তুষ্টি" : "Customer Satisfaction") : (isBn ? "ক্লিনিকাল প্রশ্ন" : "Clinical Inquiries")}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="text-[10.5px] text-slate-300/80 font-sans relative z-10 flex items-center gap-1.5">
          ...
          <span>{isBn ? "কঠোর মেডিকেল গোপনীয়তা নিশ্চিত।" : "Strict medical confidentiality protocols applied."}</span>
        </div>
      </div>
      ...
      <div className={`flex-1 flex flex-col justify-between py-12 px-4 sm:px-6 lg:px-16 relative overflow-hidden transition-all duration-700 z-10 bg-gradient-to-br ${activeClass.gradientBg}`}>
        ...
        <div className="max-w-md w-full mx-auto relative z-10">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> {isBn ? "হোমপেজে ফিরুন" : "BACK TO HOMEPAGE"}
          </button>
        </div>
        ...
        <div 
          ...
          className={`max-w-md w-full mx-auto rounded-3xl p-6 sm:p-8 transition-all duration-200 ease-out relative z-10 ${activeClass.card}`}
        >
          ...
          <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-bold uppercase tracking-wider mb-4 transition-colors duration-300 ${activeClass.pillBadge}`}>
            {isPatient ? (
              <>
                ...
                <span>{isBn ? "রোগী পোর্টাল" : "Patient Access portal"}</span>
              </>
            ) : (
              <>
                <Stethoscope className="w-3.5 h-3.5 text-blue-400" />
                <span>{isBn ? "ডাক্তার ড্যাশবোর্ড" : "Clinician Dashboard"}</span>
              </>
            )}
          </div>
          ...
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight font-serif">
            {verificationPending 
              ? (isBn ? "অ্যাকাউন্ট যাচাই" : "Verify Account") 
              : isForgotPassword 
          ...
          <p className={`text-xs mt-1.5 font-sans leading-relaxed max-w-[310px] ${activeClass.labelColor}`}>
            {verificationPending 
              ? isPatient
                ? (isBn ? "আপনার ফোনে এসএমএস এর মাধ্যমে একটি যাচাইকরণ কোড পাঠানো হয়েছে।" : "We sent a verification security code to your phone by SMS. Please check your messages.")
                : (isBn ? `আমরা ${otpEmailOrPhone} এ একটি যাচাইকরণ কোড পাঠিয়েছি। অনুগ্রহ করে আপনার ইনবক্স চেক করুন।` : `We emailed a verification security code to ${otpEmailOrPhone}. Please check your inbox.`)
              : isForgotPassword
                ? resetOtpSent
                  ? isPatient
                    ? (isBn ? "আপনার ফোনে পাঠানো রিসেট কোডটি দিন এবং একটি নতুন পাসওয়ার্ড নির্বাচন করুন।" : "Enter the reset code sent to your phone by SMS and choose a new secure password.")
                    : (isBn ? "আপনার ইমেইলে পাঠানো কোডটি দিন এবং একটি নতুন পাসওয়ার্ড নির্বাচন করুন।" : "Enter the reset code sent to your email and choose a new secure password.")
                  : isPatient
                    ? (isBn ? "পাসওয়ার্ড রিসেট কোড পেতে আপনার নিবন্ধিত ফোন/ইমেইল লিখুন।" : "Enter your registered phone/email to receive a secure password reset code.")
                    : (isBn ? "পাসওয়ার্ড রিসেট লিঙ্ক পেতে আপনার নিবন্ধিত ইমেইল লিখুন।" : "Enter your registered email address to receive a secure password reset link.")
                : (isBn ? "আপনার ব্যক্তিগতকৃত মেডিকেল পোর্টালে প্রবেশ করতে আপনার ভূমিকা নির্বাচন করুন।" : "Select your role to access your personalized medical portal.")}
          </p>
            ...
            <form onSubmit={handleVerifyOtp} className="w-full mt-6 space-y-5">
              ...
              <div>
                <label className={`block text-[10px] font-bold uppercase tracking-wider mb-2 pl-1 font-sans ${activeClass.labelColor}`}>
                  {isBn ? "যাচাইকরণ কোড" : "Verification Code"}
                </label>
                <SegmentedOTPInput
                  value={otpCode}
                  onChange={setOtpCode}
