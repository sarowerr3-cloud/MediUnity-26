...
import { MessageSquare, Calendar, ChevronRight, User, PlusCircle, Filter, Send, ArrowLeft, CheckCircle2, ThumbsUp, Image, Video, X, Paperclip, Bookmark, Share2 } from "lucide-react";
  ...
  async function fetchPosts() {
    ...
    try {
      ...
      if (json.success) {
        setPosts(json.posts);
        if (sharedPostId) {
          setActivePostCommentsId(sharedPostId);
          setTimeout(() => {
            const el = document.getElementById(`post-${sharedPostId}`);
            if (el) {
              el.scrollIntoView({ behavior: "smooth", block: "center" });
              el.classList.add("ring-4", "ring-emerald-500/30", "transition-all", "duration-500");
              setTimeout(() => {
                el.classList.remove("ring-4", "ring-emerald-500/30");
              }, 4000);
            }
          }, 300);
        }
    ...
    <div className="min-h-screen bg-linear-to-br from-green-50/50 to-emerald-100/30 flex flex-col font-serif">
      ...
      <main className="flex-grow max-w-5xl w-full mx-auto px-4 py-24">
        ...
        <div className="bg-white/60 border border-emerald-200/60 rounded-3xl p-8 mb-8 shadow-sm backdrop-blur-md flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-emerald-950 tracking-tight">
              {isBn ? "স্বাস্থ্য প্রশ্নোত্তর ও কমিউনিটি ফোরাম" : "Health Q&A Community"}
            </h1>
            <p className="text-slate-600 mt-2 text-sm sm:text-base leading-relaxed">
              {isBn
                ? "শারীরিক লক্ষণ শেয়ার করুন, স্বাস্থ্য বিষয়ে প্রশ্ন করুন এবং সার্টিফাইড বিশেষজ্ঞ চিকিৎসকদের নির্ভরযোগ্য পরামর্শ পান।"
          ...
          {isSignedIn && (
            ...
            >
              <PlusCircle className="w-5 h-5" /> {isBn ? "প্রশ্ন বা পোস্ট করুন" : "Ask a Question"}
            </button>
          )}

          {!isSignedIn && !doctorToken && (
            <div className="text-sm font-semibold bg-emerald-100 text-emerald-800 px-4 py-2 rounded-xl">
              {isBn ? "প্রশ্ন করতে লগইন করুন" : "Log in to post a question"}
            </div>
          )}
          ...
          {doctorToken && (
            <div className="text-sm font-semibold bg-blue-100 text-blue-800 px-4 py-2 rounded-xl flex items-center gap-1.5 border border-blue-200">
              <CheckCircle2 className="w-4 h-4 text-blue-600" /> {isBn ? "ডাক্তার হিসেবে লগইন করা আছে" : "Logged in as Doctor"}
            </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-emerald-200 shadow-2xl animate-fade-in">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-slate-800">
                  {isBn ? "কমিউনিটিতে প্রশ্ন করুন" : "Ask the Community"}
                </h3>
                <button 
              ...
              <form onSubmit={handleCreatePost} className="space-y-4 font-sans">
                {forumTab === "circles" ? (
                  <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-800 font-semibold">
                    🎯 {isBn ? "এই পোস্টটি নির্দিষ্ট সার্কেলে প্রকাশিত হবে: " : "This post will be published in the support circle: "} <b>{activeCircle}</b>
                  </div>
                ) : (
                  <div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                    {isBn ? "পোস্ট বা প্রশ্নের বিষয়" : "Subject / Query Title"}
                  </label>
                  <input
                    type="text"
                    placeholder={isBn ? "যেমন: ৩ দিন ধরে তীব্র কোমর ব্যথা" : "e.g. Constant lower back pain for 3 days"}
                    value={newPost.title}
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                    {isBn ? "বিস্তারিত বিবরণ" : "Details of your condition"}
                  </label>
                  <textarea
                    rows={4}
                    placeholder={isBn ? "আপনার লক্ষণসমূহ, কতদিন ধরে সমস্যা, কোনো ওষুধ সেবন করছেন কিনা ইত্যাদি বিস্তারিত লিখুন..." : "Describe your symptoms, how long you've had them, past medications, and any other helpful context..."}
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                    {isBn ? "ছবি / ভিডিও সংযুক্ত করুন (ভিডিও সর্বোচ্চ ৩০০ মেগাবাইট)" : "Attach Photo / Video (Video Max 300MB)"}
                  </label>
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {uploadingPostMedia && (
                      <span className="text-xs text-slate-500 animate-pulse font-medium">
                        {isBn ? "মিডিয়া আপলোড হচ্ছে..." : "Uploading media..."}
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  <label htmlFor="postAnonymous" className="text-xs font-bold text-slate-700 cursor-pointer">
                    {isBn ? "🤫 নাম গোপন রেখে পোস্ট করুন (আপনার নাম ও ছবি লুকানো থাকবে)" : "🤫 Post Anonymously (Hide your name & avatar)"}
                  </label>
                </div>
                ...
                <div className="flex gap-3 justify-end pt-4">
                  ...
                  >
                    {submittingPost ? (isBn ? "প্রকাশ হচ্ছে..." : "Publishing...") : (forumTab === "qa" ? (isBn ? "প্রশ্ন প্রকাশ করুন" : "Publish Query") : (isBn ? "পোস্ট প্রকাশ করুন" : "Publish Post"))}
                  </button>
                </div>
              </form>
            </div>
        ...
        <div className="flex border-b border-emerald-100 mb-8 bg-white/40 backdrop-blur-md rounded-2xl p-1.5 shadow-xs gap-1">
          ...
          >
            🩺 {isBn ? "ডাক্তারকে জিজ্ঞাসা" : "Ask a Doctor"}
          </button>
          <button
          ...
          >
            🤝 {isBn ? "সহায়তা সার্কেল" : "Support Circles"}
          </button>
        </div>
        ...
        {loading ? (
          <div className="text-center py-16 text-slate-500 font-medium">
            {isBn ? "কমিউনিটি আলোচনা ও প্রশ্নোত্তর লোড হচ্ছে..." : "Loading community questions..."}
          </div>
        ) : posts.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center text-slate-500 shadow-sm">
            ...
            <p className="font-semibold text-lg">{isBn ? "কোনো পোস্ট বা প্রশ্ন পাওয়া যায়নি" : "No queries found"}</p>
            <p className="text-sm mt-1">{isBn ? "এই ক্যাটাগরিতে প্রথম পোস্ট বা প্রশ্নটি আপনিই করুন!" : "Be the first to share a health query in this category!"}</p>
          </div>
        ) : (
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-500 mt-2 mb-4">
                    ...
                    <span>
                      ...
                      <span className="font-semibold text-slate-700 inline-flex items-center gap-1">
                        {post.authorName} {post.authorRole === "doctor" && <VerifiedBadge isVerified={post.doctorIsVerified} hideUnverified={true} size="sm" />}
                      </span>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                    <div className="flex items-center gap-6">
                      ...
                      >
                        ...
                        <span>
                          {post.comments.length} {post.isQA ? (isBn ? "উত্তর" : `Answer${post.comments.length !== 1 ? "s" : ""}`) : (isBn ? "মন্তব্য" : `Reply${post.comments.length !== 1 ? "s" : ""}`)}
                        </span>
                      ...
                      >
                        <Bookmark className={`w-4 h-4 ${savedPosts.includes(post._id) ? "fill-amber-600 text-amber-600" : ""}`} />
                    ...
                    {(forumTab === "qa" && isSignedIn) && (
                      <span className="text-xs text-slate-400 italic">
                        {isBn ? "শুধুমাত্র ভেরিফায়েড ডাক্তারগণ উত্তর দিতে পারেন" : "Only verified doctors can answer"}
                      </span>
                    )}

                    {(forumTab === "discussion" && (isSignedIn || doctorToken)) && (
                      <span className="text-xs text-slate-400 italic">
                        {isBn ? "নিচে আপনার মতামত লিখুন" : "Post a reply below"}
                      </span>
                    )}
                  ...
                  {isCommentsOpen && (
                    <div className="mt-6 border-t border-slate-100 pt-6 space-y-4 font-sans">
                      <h4 className="text-sm font-bold text-slate-700 mb-3">
                        {post.isQA ? (isBn ? "চিকিৎসকদের উত্তর" : "Medical Answers") : (isBn ? "মন্তব্য ও প্রতিক্রিয়া" : "Replies & Comments")}
                      </h4>
                      
                      {post.comments.length === 0 ? (
                        <p className="text-slate-400 text-sm italic pl-4">
                          {post.isQA 
                            ? (isBn ? "এখনও কোনো উত্তর দেওয়া হয়নি। সম্মানিত বিশেষজ্ঞ ডাক্তারগণ আপনার মতামত প্রদান করুন।" : "No professional answers yet. Verified doctors, please share your recommendations!")
                            : (isBn ? "এখনও কোনো মন্তব্য নেই। নিচে আপনার প্রতিক্রিয়া জানান!" : "No replies yet. Start the conversation below!")}
                        </p>
                      ) : (
                        <div className="space-y-4 pl-2 sm:pl-4">
                          {post.comments.map((comment) => {
                            ...
                            if (!isDoctor) {
                              ...
                              if (count >= 10) patientBadge = isBn ? "শীর্ষ কন্ট্রিবিউটর" : "Top Contributor";
                              else if (count >= 3) patientBadge = isBn ? "কমিউনিটি হেল্পার" : "Community Helper";
                            }
                                ...
                                <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                                  <div className="flex flex-wrap items-center gap-2">
                                    {comment.isAnonymous ? (
                                      ...
                                      <span className={`text-sm font-bold flex items-center gap-1 ${isDoctor ? "text-blue-900" : "text-slate-800"}`}>
                                        {comment.authorName}
                                        {isDoctor && <VerifiedBadge isVerified={comment.doctorIsVerified} hideUnverified={true} size="sm" />}
                                      </span>
                                    )}
                                    ...
                                    {isDoctor && (
                                      <>
                                        <span className="flex items-center gap-0.5 px-2.5 py-0.5 bg-blue-100 text-blue-800 text-[10px] font-extrabold rounded-full border border-blue-200">
                                          ...
                                          {isBn ? "ভেরিফায়েড" : "Verified"} {comment.doctorSpecialization || (isBn ? "বিশেষজ্ঞ" : "Specialist")}
                                        </span>
                                        ...
                                        <span className="text-[10px] text-blue-600 font-semibold bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-full">
                                          {comment.doctorReputationPoints || 0} {isBn ? "পয়েন্ট" : "Rep"}
                                        </span>
                                      </>
                                    )}
                                    ...
                                    {!isDoctor && patientBadge && (
                                      <span className={`px-2 py-0.5 text-[10px] font-extrabold rounded-full border ${
                                        patientBadge.includes("Top") || patientBadge.includes("শীর্ষ")
                                ...
                                {post.isQA && (
                                  <div className="flex items-center justify-end">
                                    ...
                                    >
                                      ...
                                      <span>{isBn ? "সহায়ক" : "Helpful"} ({commentUpvotes.length})</span>
                                    </button>
                                  </div>
                                )}
                              </div>
                      ...
                      {post.isQA ? (
                        ...
                        doctorToken ? (
                          doctorInfo?.isVerified ? (
                            <div className="flex flex-col gap-2 mt-4 pl-2 sm:pl-4">
                              ...
                              <div className="flex gap-2 items-end">
                                <textarea
                                  rows={2}
                                  placeholder={isBn ? "পেশাদার চিকিৎসা পরামর্শ বা উত্তর লিখুন..." : "Provide a professional medical answer or recommendation..."}
                                ...
                                />
                                <label className="p-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-full transition shadow cursor-pointer shrink-0" title={isBn ? "ফাইল সংযুক্ত করুন" : "Attach file"}>
                                ...
                                <button
                                  ...
                                  title={isBn ? "উত্তর পাঠান" : "Send Answer"}
                                >
                              ...
                              {uploadingCommentMedia[post._id] && (
                                <span className="text-[10px] text-slate-500 animate-pulse font-medium">
                                  {isBn ? "ফাইল আপলোড হচ্ছে..." : "Uploading file..."}
                                </span>
                              )}
                            </div>
                          ) : (
                            <div className="mt-4 mx-2 sm:mx-4 p-4 bg-amber-50 border border-amber-200 text-amber-900 text-xs rounded-2xl font-semibold">
                              {isBn 
                                ? "⚠️ আপনার ডক্টরস প্রোফাইল বর্তমানে আনভেরিফায়েড। শুধুমাত্র ভেরিফায়েড ডাক্তারগণ প্রশ্নের উত্তর দিতে পারেন। আপনার ড্যাশবোর্ডে মেডিকেল সার্টিফিকেট আপলোড করুন।"
                          ...
                          <div className="mt-4 mx-2 sm:mx-4 p-4 bg-emerald-50 border border-emerald-200 text-emerald-950 text-xs rounded-2xl font-semibold">
                            {isBn 
                              ? "🩺 শুধুমাত্র ভেরিফায়েড বিশেষজ্ঞ চিকিৎসকরা প্রশ্নের উত্তর প্রদান করতে পারেন। রোগীরা সহায়ক উত্তরে লাইক ও রেটিং দিতে পারেন।"
                        ...
                        (isSignedIn || doctorToken) ? (
                          <div className="mt-4 pl-2 sm:pl-4 space-y-2">
                            ...
                            <div className="flex gap-2 items-end">
                              <textarea
                                rows={1}
                                placeholder={isBn ? "আপনার মন্তব্য লিখুন..." : "Add your reply..."}
                                value={commentText[post._id] || ""}
                                onChange={(e) => setCommentText(prev => ({ ...prev, [post._id]: e.target.value }))}
                              ...
                              />
                              <label className="p-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-full transition shadow cursor-pointer shrink-0" title={isBn ? "ফাইল সংযুক্ত করুন" : "Attach file"}>
                                <Paperclip className="w-4 h-4" />
                                <input
                                  type="file"
                                  accept="image/*,video/*"
                              ...
                              <button
                                ...
                                disabled={submittingComment || uploadingCommentMedia[post._id]}
                                className="p-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full transition shadow hover:shadow-md disabled:opacity-50 shrink-0 cursor-pointer"
                                title={isBn ? "মন্তব্য পাঠান" : "Send reply"}
                              >
                                <Send className="w-4 h-4" />
                              </button>
                            </div>
                            ...
                            {isSignedIn && (
                              <div className="flex items-center gap-1.5 pl-1">
                                ...
                                <label htmlFor={`replyAnon-${post._id}`} className="text-xs text-slate-500 font-bold cursor-pointer">
                                  {isBn ? "🤫 নাম গোপন রেখে মন্তব্য করুন" : "🤫 Reply Anonymously (Hide your name & profile)"}
                                </label>
                              </div>
                            )}
                          </div>
                          ...
                          <div className="text-center py-2 text-slate-400 text-xs italic">
                            {isBn ? "এই আলোচনায় অংশ নিতে অনুগ্রহ করে লগইন করুন।" : "Please log in to respond to this discussion."}
                          </div>
                        )
                      )}
                    </div>
      ...
      {editingPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-lg w-full border border-emerald-200 shadow-2xl animate-fade-in font-sans">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-slate-800">
                {isBn ? "পোস্ট সম্পাদনা করুন" : "Edit Post"}
              </h3>
              <button 
            ...
            <form onSubmit={handleUpdatePost} className="space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder={isBn ? "পোস্ট বা প্রশ্নের বিষয়" : "Subject / Query Title"}
                  value={editPostTitle}
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                  {isBn ? "বিস্তারিত বিবরণ" : "Details"}
                </label>
                <textarea
                  rows={4}
                  placeholder={isBn ? "আপনার সমস্যা ও বিবরণ লিখুন..." : "Describe your condition..."}
              ...
              <div className="flex gap-3 justify-end pt-4">
                ...
                <button
                  ...
                  className="px-6 py-2.5 rounded-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold flex items-center gap-2 shadow-md hover:shadow-lg disabled:opacity-50 text-sm cursor-pointer"
                >
                  {submittingEdit ? (isBn ? "সংরক্ষণ হচ্ছে..." : "Saving...") : (isBn ? "পরিবর্তন সংরক্ষণ করুন" : "Save Changes")}
                </button>
              </div>
            </form>
          </div>
