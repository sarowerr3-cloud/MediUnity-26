    ...
    <div className="min-h-screen bg-linear-to-b from-slate-900 via-slate-950 to-emerald-950 text-slate-100 font-sans overflow-hidden">
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 p-8 rounded-3xl backdrop-blur-md hover:border-emerald-500/30 transition-all duration-300">
            ...
            <h3 className="text-xl font-bold text-slate-100">{t("landing.feature_1_title", "Patient-to-Doctor Q&A")}</h3>
            <p className="text-slate-400 mt-3 text-sm leading-relaxed">
              {t("landing.feature_1_desc", "Post health questions to the community feed. Get verified answers and clinical recommendations from registered doctors, marked by their verified practitioner badges.")}
            </p>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 p-8 rounded-3xl backdrop-blur-md hover:border-emerald-500/30 transition-all duration-300">
            ...
            <h3 className="text-xl font-bold text-slate-100">{t("landing.feature_2_title", "BM&DC Verified Doctors")}</h3>
            <p className="text-slate-400 mt-3 text-sm leading-relaxed">
              {t("landing.feature_2_desc", "Book consultations with licensed, certified healthcare specialists across top medical disciplines.")}
            </p>
          </div>
          ...
          <div className="bg-slate-900/60 border border-slate-800/80 p-8 rounded-3xl backdrop-blur-md hover:border-emerald-500/30 transition-all duration-300">
            ...
            <h3 className="text-xl font-bold text-slate-100">{t("landing.feature_3_title", "Digital Locker & Prescriptions")}</h3>
            <p className="text-slate-400 mt-3 text-sm leading-relaxed">
              {t("landing.feature_3_desc", "Keep your medical records, laboratory reports, and digital prescriptions safely stored in the cloud.")}
            </p>
          </div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-16 border-t border-slate-800/50">
        <div className="text-center mb-12">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-100">{t("doctors.title", "Meet Verified Specialists On-Call")}</h2>
          <p className="text-slate-400 mt-2 text-sm">{t("doctors.subtitle", "Consult licensed practitioners directly through scheduled appointment slots.")}</p>
        </div>

        {loading ? (
          <div className="text-center text-slate-500">{t("common.loading", "Loading specialist registry...")}</div>
        ) : (
            ...
            {doctors.map((doc) => (
              ...
              >
                ...
                <div className="flex items-center gap-1 text-amber-400 text-xs mt-3">
                  <Star className="w-3.5 h-3.5 fill-amber-400" /> {doc.rating || "5.0"} • {doc.experience || "5"} {t("doctors.experience", "Years Exp")}
                </div>
                ...
                >
                  {t("doctors.book_appointment", "View Profile & Book")}
                </Link>
              </div>
            ))}
          </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-16 mb-20">
        <div className="bg-linear-to-r from-emerald-950/80 to-slate-900/90 border border-emerald-500/20 p-8 rounded-3xl text-center backdrop-blur-md">
          ...
          <h3 className="text-2xl font-bold text-slate-100">{t("app.global_standard", "Private, Secure, Compliance Checked")}</h3>
          <p className="text-slate-400 mt-2 text-sm leading-relaxed max-w-xl mx-auto">
            {t("app.operated_under", "Your personal information is encrypted. We enforce automatic doctor credentials checking and medical verification to keep the social sphere safe.")}
          </p>
        </div>
      </section>
    </div>
