...
import { BookOpen, Calendar, MessageSquare, Heart, Bookmark, PlusCircle, Filter, CheckCircle2, User, Clock, ArrowRight, Share2 } from "lucide-react";
...
export default function Articles() {
  ...
  async function handleToggleBookmark(articleId, reference) {
    if (!isSignedIn) {
    ...
    try {
      ...
      if (json.success) {
        ...
        toast.error(json.message || "Failed to save bookmark");
      }
    } catch (err) {
  ...
  const handleShareArticle = (articleId) => {
    const shareUrl = `${window.location.origin}/articles/${articleId}`;
    navigator.clipboard.writeText(shareUrl)
      .then(() => {
        toast.success(isBn ? "লিঙ্কটি ক্লিপবোর্ডে কপি করা হয়েছে!" : "Article link copied to clipboard!");
      })
      .catch(() => {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-green-50/50 to-emerald-100/30 flex flex-col font-serif">
      ...
      <main className="flex-grow max-w-5xl w-full mx-auto px-4 py-24">
        ...
        <div className="bg-white/60 border border-emerald-200/60 rounded-3xl p-8 mb-8 shadow-sm backdrop-blur-md flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-emerald-950 tracking-tight">
              {isBn ? "মেডিকেল হেলথ হাব ও ব্লগ" : "Medical Health Hub"}
            </h1>
            <p className="text-slate-600 mt-2 text-sm sm:text-base leading-relaxed">
              {isBn
                ? "আমাদের সার্টিফাইড বিশেষজ্ঞ চিকিৎসকদের দ্বারা রচিত নির্ভরযোগ্য স্বাস্থ্য আর্টিকেল ও গাইড পড়ুন।"
          ...
          {doctorInfo?.isVerified && (
            ...
            >
              <PlusCircle className="w-5 h-5" /> {isBn ? "আর্টিকেল লিখুন" : "Write an Article"}
            </button>
          )}

          {!doctorToken && !isSignedIn && (
            <div className="text-sm font-semibold bg-emerald-100 text-emerald-800 px-4 py-2 rounded-xl">
              {isBn ? "যাচাইকৃত বিশেষজ্ঞদের স্বাস্থ্য ব্লগ" : "Medical blogs by verified specialists"}
            </div>
          )}
          ...
          {doctorToken && !doctorInfo?.isVerified && (
            <div className="text-sm font-semibold bg-amber-50 text-amber-800 border border-amber-200 px-4 py-2 rounded-xl max-w-xs text-center">
              ⚠️ {isBn ? "আর্টিকেল প্রকাশের জন্য প্রোফাইল ভেরিফাই করুন" : "Verify your account to publish articles"}
            </div>
          )}
        </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-2xl w-full border border-emerald-200 shadow-2xl animate-fade-in font-sans">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                  <BookOpen className="text-emerald-600 w-5 h-5" /> {isBn ? "নতুন আর্টিকেল তৈরি করুন" : "Create Article"}
                </h3>
                <button
              ...
              <form onSubmit={handleCreateArticle} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                      {isBn ? "আর্টিকেল শিরোনাম" : "Article Title"}
                    </label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1.5">
                    {isBn ? "আর্টিকেলের বিষয়বস্তু" : "Body Content"}
                  </label>
                  <textarea
                    rows={10}
                    placeholder={isBn ? "আপনার তথ্যবহুল আর্টিকেল এখানে লিখুন। স্বাস্থ্য সচেতনতামূলক টিপস, ক্লিনিক্যাল গাইডলাইন ইত্যাদি যুক্ত করুন..." : "Write your informative article here. Provide credible health tips, clinical guidelines, or research summaries..."}
                ...
                <div className="flex gap-3 justify-end pt-4">
                  ...
                  >
                    {submittingArticle ? (isBn ? "প্রকাশ হচ্ছে..." : "Publishing...") : (isBn ? "আর্টিকেল প্রকাশ করুন" : "Publish Article")}
                  </button>
                </div>
              </form>
            </div>
        ...
        {loading ? (
          <div className="text-center py-16 text-slate-500 font-medium font-sans">
            {t("common.loading", "Loading health hub...")}
          </div>
        ) : articles.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center text-slate-500 shadow-sm">
            ...
            <p className="font-semibold text-lg">{isBn ? "কোনো আর্টিকেল পাওয়া যায়নি" : "No articles found"}</p>
            <p className="text-sm mt-1 font-sans">{isBn ? "ডাক্তারগণ আর্টিকেল প্রকাশ করলে এখানে প্রদর্শিত হবে।" : "Verified articles will appear here once published by our doctors."}</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => handleShareArticle(article._id)}
                          className="p-1.5 rounded-full border border-slate-200 text-slate-400 hover:text-blue-600 hover:bg-slate-50 transition duration-300 cursor-pointer"
                          title={isBn ? "নিবন্ধ শেয়ার করুন" : "Share Article"}
                        >
                  ...
                  <div className="border-t border-slate-50 mt-6 pt-4 flex items-center justify-between">
                    ...
                    >
                      {isBn ? "পড়ুন" : "Read"} <ArrowRight className="w-3.5 h-3.5" />
                    </Link>
                  </div>
                </div>
              );
      ...
      <SaveReferenceModal
        ...
        onSave={(ref) => {
          if (pendingArticleId) {
            handleToggleBookmark(pendingArticleId, ref);
          }
        }}
