...
export const navbarStylesDr = {
  // Main navbar
  navContainer: "fixed top-6 left-1/2 -translate-x-1/2 z-50 w-full md:max-w-2xl lg:max-w-4xl pl-4 pr-8 py-1 rounded-full bg-slate-950/85 backdrop-blur-md border border-emerald-500/20 shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex items-center justify-between gap-3 transition-all duration-300 hover:shadow-emerald-500/10 hover:-translate-y-0.5",
  ...
  brandTitle: "text-3xl text-white font-semibold tracking-wide",
  brandSubtitle: "text-xs text-white/80",
  ...
  activeLink: "bg-med-soft text-white shadow-[0_0_15px_rgba(16,185,129,0.3)] scale-105 ring-2 ring-med-soft/20",
  inactiveLink: "text-white hover:bg-white/10 hover:text-white hover:-translate-y-0.5 hover:shadow",
  ...
  logoutButtonDesktop: "hidden lg:flex items-center gap-2 px-4 py-2 rounded-full bg-med-soft text-red-600 border border-white/20 shadow-sm text-sm font-semibold transition-all duration-200 transform hover:scale-105 hover:-translate-y-0.5 hover:bg-med-soft/90",
  
  // Hamburger menu buttons
  hamburgerButtonMd: "md:hidden p-2 rounded-md hover:bg-white/10 text-white transition-colors",
  hamburgerButtonLg: "hidden md:flex lg:hidden p-2 rounded-md hover:bg-white/10 text-white transition-colors",
  
  // Mobile/tablet menu
  mobileMenuContainer: (isOpen) => 
    `lg:hidden fixed top-24 left-1/2 -translate-x-1/2 z-40 w-[92%] max-w-md bg-slate-950/95 backdrop-blur-lg border border-emerald-500/20 rounded-2xl shadow-2xl transform origin-top transition-all duration-300 ${isOpen ? "scale-100 opacity-100 translate-y-0" : "scale-95 opacity-0 -translate-y-2 pointer-events-none"}`,
  ...
  mobileActiveLink: "bg-med-soft text-white",
  mobileInactiveLink: "text-white hover:bg-white/10",
  
  // Mobile logout button
  mobileLogoutButton: "mt-2 px-4 py-2 rounded-full text-center bg-med-soft text-red-600 font-semibold text-sm shadow-sm transition-transform duration-150 hover:scale-105 hover:bg-med-soft/90 w-full",
...
export const editProfilePageStyles = {
  // Page container
  pageContainer: "min-h-screen font-serif bg-med-lightest text-gray-855 p-4 sm:p-5 md:p-6",
  maxWidthContainer: "max-w-6xl pt-8 md:pt-10 mx-auto relative",
  
  // Loading states
  loadingContainer: "min-h-screen flex items-center justify-center",
  loadingSpinner: "w-12 h-12 border-4 border-gray-200 border-t-transparent rounded-full animate-spin mx-auto mb-4",
  ...
  toastSuccess: "bg-white border-l-4 border-med-soft",
  toastError: "bg-rose-50 border-l-4 border-rose-500",
  toastInfo: "bg-white border-l-4 border-blue-500",
  toastIcon: "w-5 h-5 mt-0.5",
  toastSuccessIcon: "text-med-soft",
  toastErrorIcon: "text-rose-500",
  toastText: "text-sm font-medium text-gray-800",
  
  // Main card
  mainCard: "bg-white rounded-3xl shadow-2xl overflow-hidden border border-med-light",
  headerBackground: "relative h-24 sm:h-28 md:h-32 bg-med-dark",
  ...
  imageEditButton: (editing) => `absolute bottom-2 right-2 bg-white border border-med-light rounded-full p-2 shadow-lg cursor-pointer transition-transform ${!editing && "cursor-not-allowed"}`,
  imageEditIcon: (editing) => `w-5 h-5 ${editing ? "text-med-soft" : "text-gray-400"}`,
  imageInput: "hidden",
  ...
  profileName: "text-2xl sm:text-3xl md:text-4xl font-bold bg-linear-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent truncate",
  profileSubtitle: "text-sm sm:text-lg text-gray-700 mt-2 flex items-center gap-2",
  subtitleIcon: "w-4 h-4",
  
  // Stats container
  statsContainer: "mt-4 flex flex-wrap items-center gap-3",
  statItem: "flex items-center gap-3 bg-med-lightest px-3 py-2 rounded-full border border-med-light shadow-sm text-sm sm:text-base",
  ratingStatItem: "flex items-center gap-3 bg-white px-3 py-2 rounded-full border border-med-light text-sm sm:text-base",
  feeStatItem: "flex items-center gap-1 bg-white px-3 py-2 rounded-full border border-med-light",
  statIcon: "w-4 h-4",
  statEmeraldIcon: "text-med-soft",
  statAmberIcon: (field) => {
    if (field === 'star') return "w-5 h-5 text-amber-500 fill-amber-500";
    return "w-4 h-4 text-med-soft";
  },
  statLabel: "text-xs text-med-soft font-medium",
  statAmberLabel: "text-xs text-med-soft font-medium",
  statValue: "text-sm font-semibold text-gray-800 truncate",
  statAmberValue: "text-sm font-bold text-gray-800",
  statInput: "w-20 rounded-full border px-2 py-1 text-sm bg-white border-med-light text-gray-800 focus:outline-none",
  statPatientsInput: "w-24 rounded-full border px-2 py-1 text-sm bg-white border-med-light text-gray-800 focus:outline-none",
  
  // Action buttons
  actionButtons: "flex flex-col sm:flex-row items-center gap-3",
  availabilityToggle: (isAvailable) => `flex items-center gap-3 px-4 sm:px-5 py-2 rounded-full cursor-pointer border-2 shadow-sm transition-all duration-300 ${isAvailable ? "bg-white border-med-light hover:shadow-med-soft/20" : "bg-gray-50 border-gray-200 hover:shadow-black"} hover:shadow-lg w-full sm:w-auto`,
  toggleTrack: (isAvailable) => `relative w-10 h-5 rounded-full transition-colors ${isAvailable ? "bg-med-soft" : "bg-gray-300"}`,
  toggleThumb: (isAvailable) => `absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${isAvailable ? "left-6" : "left-0.5"}`,
  toggleText: (isAvailable) => `font-medium ${isAvailable ? "text-gray-850" : "text-gray-500"}`,
  
  editButton: "group relative overflow-hidden bg-med-soft hover:opacity-90 text-white px-5 py-2 rounded-full cursor-pointer shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-[1.02] w-full sm:w-auto",
  editButtonContent: "relative flex items-center gap-2",
  
  // Form sections
  formSection: "mb-8",
  sectionTitle: "text-xl sm:text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2",
  sectionIconContainer: "w-8 h-8 rounded-full bg-med-lightest flex items-center justify-center",
  sectionIcon: "w-4 h-4 text-med-soft",
  
  // Field grid
  fieldGrid: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6",
  fieldGroup: "group",
  fieldHeader: "flex items-center gap-3 mb-2",
  fieldIconContainer: (editing) => `p-2 rounded-full ${editing ? "bg-med-lightest text-med-soft" : "bg-gray-150 text-gray-400"}`,
  fieldIcon: "w-4 h-4",
  fieldLabel: "text-sm font-semibold text-gray-700",
  
  // Input fields
  inputBase: (editing) => `w-full rounded-full border-2 px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base transition-all duration-200 ${editing ? "border-med-light bg-white text-gray-800 focus:border-med-soft focus:ring-2 focus:ring-med-soft/20 focus:bg-white" : "border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed"}`,
  
  // About textarea
  aboutTextarea: (editing) => `w-full rounded-xl border-2 px-4 py-3 text-sm sm:text-base transition-all duration-200 ${editing ? "border-med-light bg-white text-gray-800 focus:border-med-soft focus:ring-2 focus:ring-med-soft/20 focus:bg-white" : "border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed"}`,
  aboutCharCount: "absolute bottom-3 right-3 text-xs text-gray-500",
  
  // Schedule section
  scheduleHeader: "flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6",
  emptySchedule: "text-center py-10 rounded-2xl border-2 border-dashed border-med-light bg-white",
  emptyScheduleIcon: "w-12 h-12 text-med-soft mx-auto mb-3",
  emptyScheduleText: "text-gray-800 font-medium",
  emptyScheduleSubtext: "text-sm text-gray-500 mt-1",
  
  // Schedule grid
  scheduleGrid: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5",
  
  // Date card
  dateCard: "group relative bg-white p-4 sm:p-5 rounded-2xl border border-med-light shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-1",
  dateHeader: "flex items-center justify-between mb-3 pb-3 border-b border-med-light",
  dateIconContainer: "p-2 rounded-full bg-med-lightest",
  dateIcon: "w-5 h-5 text-med-soft",
  dateTitle: "font-bold text-base sm:text-lg text-gray-900",
  dateSubtitle: "text-xs sm:text-sm text-med-soft",
  dateSlotCount: "text-xs font-medium px-3 py-1 rounded-full bg-med-lightest text-gray-700",
  dateDeleteButton: (editing) => `p-2 rounded-full cursor-pointer transition-colors ${editing ? "hover:bg-rose-50 text-rose-550" : "text-gray-300 cursor-not-allowed"}`,
  dateDeleteIcon: "w-4 h-4",
  
  // Time slots
  timeSlotContainer: "space-y-3",
  timeSlotItem: "flex items-center justify-between bg-med-lightest px-3 py-2 rounded-full border border-med-light hover:border-med-soft transition-colors",
  timeSlotIcon: "w-4 h-4 text-med-soft",
  timeSlotText: "font-medium text-gray-800 text-sm sm:text-base",
  timeSlotDeleteButton: (editing) => `p-1.5 rounded-full cursor-pointer transition-colors ${editing ? "hover:bg-rose-50 text-rose-550" : "text-gray-300 cursor-not-allowed"}`,
  timeSlotDeleteIcon: "w-4 h-4",
  
  // Add time slot
  addSlotContainer: "pt-3 border-t border-med-light",
  addSlotInput: "flex-grow rounded-full px-3 py-2 text-sm border border-med-light bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-med-soft/20",
  addSlotButton: "p-2 rounded-full cursor-pointer bg-med-lightest text-med-soft hover:bg-med-light transition-colors",
  addSlotIcon: "w-4 h-4",
  
  // Save message
  saveMessage: (type) => `px-4 py-2 rounded-lg bg-white border border-med-light text-gray-800`,
  ...
  resetButton: "px-6 py-3 rounded-full cursor-pointer border-2 border-med-light text-gray-700 hover:bg-med-lightest hover:border-med-soft transition-all duration-200 font-medium w-full sm:w-auto text-center",
  saveButton: "group relative overflow-hidden bg-med-soft hover:opacity-90 text-white px-6 py-3 rounded-full cursor-pointer shadow-lg transition-all duration-300 hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed w-full sm:w-auto text-center",
  ...
  addDateInput: "rounded-xl px-3 py-2 border-2 border-med-light bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-med-soft/20 focus:border-med-soft text-sm sm:text-base",
  addDateButton: "flex items-center gap-2 bg-med-soft text-white px-4 py-2 rounded-xl shadow-md hover:opacity-90 transition-all duration-200 text-sm sm:text-base",
  addDateIcon: "w-4 h-4",
...
export const dashboardStyles = {
  // Main container
  pageContainer: "min-h-screen font-serif pt-16 lg:pt-20 md:pt-15 p-4 sm:p-6 bg-med-lightest text-gray-800",
  ...
  headerSubtitle: "text-sm sm:text-base text-gray-500",
  headerInfo: "text-sm text-gray-500",
  refreshButton: "text-sm px-3 py-1 rounded-full bg-white text-med-soft border border-med-light hover:bg-med-lightest",
  
  // Stats grid
  statsGrid: "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8",
  
  // Stat card
  statCard: "rounded-full p-4 bg-white border border-med-light shadow-sm hover:shadow-md transition-all",
  statContent: "flex items-center justify-between gap-4",
  statTextContainer: "flex-1",
  statTitle: "text-sm font-medium text-gray-600",
  statValue: "mt-2 text-xl sm:text-2xl font-extrabold text-gray-900 tracking-tight",
  statIconContainer: "p-3 rounded-full border shadow-md bg-med-lightest border-med-light text-med-soft",
  statIcon: "w-6 h-6",
  ...
  accentBottomEmerald: "border-med-light",
  accentBottomAmber: "border-med-light",
  accentBottomRose: "border-med-light",
  
  // Appointments container
  appointmentsContainer: "bg-white rounded-2xl p-4 border border-med-light shadow-sm",
  appointmentsHeader: "flex items-center justify-between mb-4",
  appointmentsTitle: "text-lg sm:text-xl font-semibold text-gray-900",
  appointmentsTotal: "text-sm sm:text-base text-gray-700 flex items-center gap-2",
  totalIcon: "w-4 h-4",
  ...
  // Appointment card
  appointmentCard: "rounded-xl p-4 bg-med-lightest/45 shadow-sm border border-med-light flex flex-col justify-between gap-4 hover:shadow-md transition self-start",
  cardHeader: "flex items-start gap-3",
  cardAvatar: "w-12 h-12 sm:w-14 sm:h-14 rounded-full overflow-hidden bg-white border border-med-light flex items-center justify-center",
  cardAvatarImage: "w-full h-full object-cover",
  cardAvatarFallback: "text-gray-700 font-bold",
  cardContent: "flex-1",
  cardPatientName: "text-base sm:text-lg font-bold text-gray-900",
  cardPatientInfo: "text-xs sm:text-sm text-gray-600 mt-1",
  cardDoctorInfo: "mt-2 text-sm sm:text-sm text-gray-750",
  cardDoctorName: "font-semibold text-gray-900",
  cardSpeciality: "text-xs sm:text-sm text-med-soft font-medium",
  cardPhoneContainer: "mt-2 text-xs sm:text-sm text-med-soft flex items-center gap-2",
  cardPhoneIcon: "w-4 h-4",
  
  // Date and time section
  dateTimeContainer: "flex items-center justify-between",
  dateText: "text-sm sm:text-lg font-bold text-gray-850",
  timeText: "text-sm sm:text-base font-semibold text-gray-755",
  
  // Card footer
  cardFooter: "flex flex-col items-end gap-2",
  feeText: "text-sm sm:text-base text-gray-655 font-medium",
  statusContainer: "flex items-center gap-2",
  
  // Show more button
  showMoreContainer: "mt-4 flex justify-center",
  showMoreButton: "px-4 py-2 rounded-full bg-med-soft text-white text-sm font-semibold hover:opacity-90 transition",
  
  // Status badge (reusing listPageStyles but adding here for completeness)
  statusBadgeBase: "px-3 py-1 rounded-full text-xs font-semibold",
  statusBadgeComplete: "bg-gray-150 text-gray-700 border border-gray-200",
  statusBadgeCancelled: "bg-rose-50 text-rose-700 border border-rose-200",
  statusBadgeConfirmed: "bg-blue-50 text-blue-755 border border-blue-200",
  statusBadgeRescheduled: "bg-purple-50 text-purple-755 border border-purple-200",
  statusBadgePending: "bg-yellow-50 text-amber-700 border border-amber-200 animate-pulse",
  
  // Status select
  statusSelect: "text-xs sm:text-sm px-3 py-1 rounded-full border focus:outline-none transition",
  statusSelectEnabled: "bg-white text-gray-800 border-med-light hover:shadow-sm",
  statusSelectDisabled: "bg-gray-100 text-gray-400 cursor-not-allowed border-gray-200",
  
  // Reschedule button
  rescheduleButton: "text-xs px-3 py-1 rounded-full border transition",
  rescheduleButtonEnabled: "bg-white text-med-dark border-med-light hover:shadow-sm",
  rescheduleButtonDisabled: "bg-gray-100 text-gray-400 border-gray-250 cursor-not-allowed",
  
  // Reschedule form
  rescheduleForm: "flex flex-col items-end gap-2 w-full",
  rescheduleDateInput: "text-xs px-3 py-2 rounded-full border border-med-light bg-white text-gray-800 w-full md:w-48 lg:w-56",
  rescheduleTimeInput: "text-xs px-3 py-2 rounded-full border border-med-light bg-white text-gray-800 w-full md:w-48 lg:w-56",
  rescheduleButtons: "flex gap-2",
  saveButton: "text-xs px-3 py-2 rounded-full bg-med-soft text-white font-semibold hover:opacity-90 transition",
  cancelButton: "text-xs px-3 py-2 rounded-full border border-med-light bg-white text-gray-655 hover:bg-med-lightest transition"
};