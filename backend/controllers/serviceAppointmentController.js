...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    const tranId = `SVC-${Date.now()}`;
    const successUrl = `${req.protocol}://${req.get("host")}/api/service-appointments/aamarpay/callback`;
    const failUrl = `${frontendBase}/service-appointment/cancel`;
    ...
    const aamarPayData = {
      ...
      desc: `Service: ${resolvedServiceName}`,
    ...
    console.log("aamarPay Service Request:", JSON.stringify(aamarPayData, null, 2));

    try {
      const response = await axios.post(AAMARPAY_URL, aamarPayData);
      console.log("aamarPay Service Response:", response.data);
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    const appt = await ServiceAppointment.findOne({ "payment.sessionId": session_id });
...
export const handleAamarpayServiceCallback = async (req, res) => {
  try {
    console.log("AamarPay Service Callback Body:", req.body);
    console.log("AamarPay Service Callback Query:", req.query);

    const { pay_status, mer_txnid, amount, currency } = req.body;
    
    // Find the appointment to get the return URL
    const apptLookup = await ServiceAppointment.findOne({ "payment.sessionId": mer_txnid });
    const frontendBase = apptLookup?.payment?.meta?.returnUrl || process.env.FRONTEND_URL || "http://localhost:5173";

    if (pay_status === "Successful" || pay_status === "Success") {
      const appt = await ServiceAppointment.findOneAndUpdate(
        { "payment.sessionId": mer_txnid },
        {
          $set: {
            "payment.status": "Paid",
      ...
      console.warn("Service Payment not successful. Status:", pay_status);
    }
    return res.redirect(`${frontendBase}/service-appointment/cancel`);

  } catch (err) {
    console.error("aamarPay service callback error:", err);
