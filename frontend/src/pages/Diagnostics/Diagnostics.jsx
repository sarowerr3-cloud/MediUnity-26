...
export default function DiagnosticsPage() {
  ...
  const filteredDiagnostics = useMemo(() => {
    let filtered = diagnostics.filter(diag => 
      diag.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      diag.address?.city?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      diag.address?.street?.toLowerCase().includes(searchQuery.toLowerCase())
    );

    if (userLocation) {
      filtered = filtered.map(d => {
        let distance = null;
        if (d.locationGeo?.coordinates?.length === 2 && d.locationGeo.coordinates[0] !== 0) {
          const docLng = d.locationGeo.coordinates[0];
          const docLat = d.locationGeo.coordinates[1];
    ...
    return filtered;
  }, [diagnostics, searchQuery, userLocation]);
        ...
        {selectedDetailDiag ? (
          ...
          <div className="space-y-8 animate-fadeIn">
            ...
            <div>
              ...
              >
                <ArrowLeft size={16} /> Back to Diagnostic Labs
              </button>
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="bg-purple-500 text-[10px] text-white font-extrabold uppercase px-2 py-0.5 rounded-full border border-purple-400/30 tracking-wider shrink-0">Verified Lab</span>
                    </div>
                    <p className="text-slate-500 mt-1 text-sm flex items-center gap-3 flex-wrap">
                      <span>📞 Contact: {selectedDetailDiag.contactPhone || "Not specified"}</span>
                      <a
                    ...
                    <div className="flex items-center gap-1.5 mt-2">
                      ...
                      >
                        ({selectedDetailDiag.reviewsCount || 0} reviews)
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              <div className="lg:col-span-2 space-y-6">
                <h3 className="text-md font-bold text-slate-700 uppercase tracking-wider border-b pb-2">Available Pathological Tests</h3>
                {(!selectedDetailDiag.testsCatalog || selectedDetailDiag.testsCatalog.length === 0) ? (
                  <p className="text-slate-400 text-xs italic">No tests listed yet.</p>
                ) : (
                    ...
                    {selectedDetailDiag.testsCatalog.map((test, index) => (
                      <div key={index} className="py-4 flex justify-between items-center gap-4 flex-wrap">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400 mt-1">Category: {test.category || "General Pathology"}</p>
                          {test.preparationRequired && (
                            <p className="text-[9px] text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100/50 w-max mt-2 font-semibold">
                              ⚠️ Preparation: {test.preparationRequired}
                            </p>
                          )}
                        </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Facilities Support</h3>
                  <div className="bg-slate-50 border rounded-2xl p-4 text-center">
                    <p className="text-sm font-extrabold text-slate-800">
                      Fully Automated Testing & Fast Online Reports
                    </p>
                    <p className="text-[9px] text-purple-600 font-bold uppercase mt-1">24 Hour Turnaround</p>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-white border border-purple-100 rounded-3xl p-6 md:p-8 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight font-serif">Diagnostic Labs</h1>
                <p className="text-slate-500 text-sm font-medium mt-2 max-w-lg">Find verified laboratory networks and book medical checkups or tests online.</p>
              </div>
              ...
              <div className="relative w-full md:max-w-md flex flex-col sm:flex-row items-center gap-2">
                <div className="relative flex-grow w-full">
                  ...
                  <input
                    type="text"
                    placeholder="Search diagnostic labs by name, city, or location..."
                    value={searchQuery}
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <button
                    ...
                    className="px-4 py-3.5 bg-purple-600 hover:bg-purple-700 text-white rounded-2xl text-sm font-bold flex items-center justify-center gap-2 cursor-pointer transition shadow-sm border border-purple-700 shrink-0 flex-1 sm:flex-none disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="px-5 py-3.5 bg-purple-100 hover:bg-purple-200 text-purple-800 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 cursor-pointer transition shadow-sm border border-purple-200 shrink-0 flex-1 sm:flex-none"
                  >
                    <MapPin className="w-4 h-4" />
            ...
            {loading ? (
              <div className="text-center py-24 text-slate-400 font-mono text-sm tracking-wider uppercase">Loading laboratory networks...</div>
            ) : filteredDiagnostics.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-3xl p-16 text-center shadow-sm">
                <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-slate-100">
                  <Search className="w-8 h-8 text-slate-300" />
                </div>
                <p className="text-slate-500 font-medium">No diagnostic labs match your search criteria.</p>
                <button onClick={() => setSearchQuery("")} className="mt-4 px-6 py-2.5 bg-purple-50 text-purple-700 font-bold text-xs rounded-xl hover:bg-purple-100 transition cursor-pointer">Clear Filters</button>
              </div>
            ) : viewMode === 'map' ? (
            ...
            {filteredDiagnostics.map((diag) => (
              ...
              >
                <div className="space-y-6">
                    ...
                    <div className="flex gap-4 items-center cursor-pointer" onClick={() => setSelectedDetailDiag(diag)}>
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-wider font-semibold">
                          📞 Contact: {diag.contactPhone || "Not specified"}
                        </p>
                        {diag.distance !== undefined && diag.distance !== null && (
                          <div className="mt-1 inline-flex items-center gap-1 text-[10px] text-purple-600 font-bold bg-purple-50 px-2 py-0.5 rounded border border-purple-100">
                            <MapPin size={10} />
                            {diag.distance.toFixed(1)} km away
                          </div>
                        )}
                        <div className="flex items-center gap-1.5 mt-2">
                          <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />
                  ...
                  <div className="space-y-3">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Available Pathological Tests</h4>
                    {(!diag.testsCatalog || diag.testsCatalog.length === 0) ? (
                      <p className="text-slate-400 text-xs italic pl-2">No tests added by this facility yet.</p>
                    ) : (
                        ...
                        {diag.testsCatalog.map((test, index) => (
                          <div key={index} className="py-3.5 flex justify-between items-center gap-4 flex-wrap">
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-slate-400 mt-0.5 line-clamp-1">Category: {test.category || "General Pathology"}</p>
                              {test.preparationRequired && (
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-slate-100 mt-4">
                    <span className="text-[10px] text-slate-400 font-semibold uppercase">
                      Total Tests: {diag.testsCatalog ? diag.testsCatalog.length : 0} Available
                    </span>
                    <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-md w-full border border-blue-200 shadow-2xl space-y-5">
            <div className="flex justify-between items-center border-b pb-3 shrink-0">
              <div>
                <h3 className="text-lg font-bold text-slate-800">Book Diagnostic Test</h3>
