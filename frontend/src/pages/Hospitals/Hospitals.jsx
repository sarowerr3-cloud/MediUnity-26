  ...
  const filteredHospitals = useMemo(() => {
    ...
    if (userLocation) {
      filtered = filtered.map(h => {
        let distance = null;
        if (h.locationGeo?.coordinates?.length === 2 && h.locationGeo.coordinates[0] !== 0) {
          const docLng = h.locationGeo.coordinates[0];
          const docLat = h.locationGeo.coordinates[1];
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex gap-4 items-center cursor-pointer" onClick={() => setSelectedDetailHospital(hosp)}>
                      ...
                      <div>
                        ...
                        {hosp.distance !== undefined && hosp.distance !== null && (
                          <div className="mt-1 inline-flex items-center gap-1 text-[10px] text-emerald-600 font-bold bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
                            <MapPin size={10} />
